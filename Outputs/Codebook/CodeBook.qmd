...
# Overview #
This codebook is designed to inform the coding process for my Torrential Twitter trial study. Prior to coding Tweets, the raw data sets were initially cleaned and tested using the statistical programming software R [@r], utilizing functions from `Janitor` [@rJanitor] and `testthat` [@testthat]. Tweets will then be coded using an application designed with Shiny [@shiny] to determine the severity of harassment and type of account. 
...
: Sample of a Tweet containing **Name-calling and gender-based insults** sent to former MP Catherine McKenna. {#tbl-nc_gender_insult-example}
